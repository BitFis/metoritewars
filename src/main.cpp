...
int main()
{
	...
	*/
	IAnimatedMesh* mesh = smgr->getMesh("C:/irrlicht-1.7.3/media/sydney.md2");
	if (!mesh)
	{
		device->drop();
		return 1;
	...
	*/
	if (node)
	{
		...
		node->setMaterialTexture( 0, driver->getTexture("C:/irrlicht-1.7.3/media/sydney.bmp") );
	}
